...
public class BinarySearchTree<E extends Comparable<E>> {
	...
	public void delete(E target) {
		...
		if (link.left() != null && link.right() != null) {
			...
			// 왼쪽 자식이 null인 가장 첫번째 노드가 가장 작은 노드
			...
			// 후계자 노드의 자식 노드가 1개만(오른쪽 자식 노드만 존재) 존재하는 경우
	...
	public List<E> preOrder() {
		...
		_Stack<Node<E>> stack = new _Stack<Node<E>>();
		List<E> result = new ArrayList<E>();
	...
	public List<E> postOrder() {
		...
		_Stack<Node<E>> stack = new _Stack<Node<E>>();
		_Stack<E> resultStack = new _Stack<E>();
		List<E> result = new ArrayList<E>();
		
		stack.push(root);
		while (!stack.isEmpty()) {
			Node<E> pointer = stack.pop();
			resultStack.push(pointer.data());
